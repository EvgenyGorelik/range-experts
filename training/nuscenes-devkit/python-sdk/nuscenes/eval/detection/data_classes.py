...
class DistDetectionMetricData(DetectionMetricData):
    def __init__(self, recall, precision, confidence, trans_err, vel_err, scale_err, orient_err, attr_err, num_gt, num_pred):
        super().__init__(recall, precision, confidence, trans_err, vel_err, scale_err, orient_err, attr_err)
        self.num_gt = num_gt
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(recall=np.array(content['recall']),
                   ...
                   orient_err=np.array(content['orient_err']),
                   attr_err=np.array(content['attr_err']),
                   num_gt=content['num_gt'],
                   num_pred=content['num_pred'])
    ...
    def random_md(cls):
        ...
        return cls(recall=np.linspace(0, 1, cls.nelem),
                   ...
                   attr_err=np.random.random(cls.nelem),
                   num_gt=cls.nelem,
                   num_pred=np.random.randint(0,cls.nelem))
...
class DistDetectionMetrics(DetectionMetrics):

    def __init__(self, cfg, dist_range: Tuple[float, float]):
        super().__init__(cfg)
        self.dist_range = dist_range

    def serialize(self):
        return {**{'dist_range_from': self.dist_range[0], 'dist_range_to': self.dist_range[1]}, **super().serialize()}
...
class DistDetectionMetricDataList(DetectionMetricDataList):
    def __init__(self, dist_range: Tuple[float, float], num_gt: int = 0):
        super().__init__()
        self.dist_range = dist_range
        self.num_gt = num_gt

    def serialize(self):
        return {**{'dist_range_from': self.dist_range[0], 'dist_range_to': self.dist_range[1], 'num_gt': self.num_gt}, **super().serialize()}
    

    @classmethod
    def deserialize(cls, content):
        dist_range = (content.pop('dist_range_from'), content.pop('dist_range_to'))
        num_gt = (content.pop('num_gt'))
        val = super().deserialize(content)
        val.dist_range = dist_range
        val.num_gt = num_gt
        return val