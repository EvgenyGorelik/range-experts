...
@MODELS.register_module()
class DistanceBasedVFE(nn.Module):
    """Distance based Voxel feature encoder.
    
    Uses dymanic voxel sizes based on distance of a point to origin 
    ...
    def __init__(self,
                 ...
                 with_cluster_center: bool = False,
                 with_voxel_center: bool = False,
                 initial_voxel_size: Tuple[float] = (0.2, 0.2, 4),
                 point_cloud_range: Tuple[float] = (0, -40, -3, 70.4, 40, 1),
                 norm_cfg: dict = dict(type='BN1d', eps=1e-3, momentum=0.01),
                 mode: str = 'max',
                 fusion_layer: dict = None,
        ...
        super(DistanceBasedVFE, self).__init__()
        assert mode in ['avg', 'max']
        assert distribution in ['linear', 'exponential']
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
        if with_voxel_center:
        ...
        self._with_voxel_center = with_voxel_center
        self.return_point_feats = return_point_feats
        self.initial_vx = initial_voxel_size[0]
        self.initial_vy = initial_voxel_size[1]        
        self.initial_vz = initial_voxel_size[2]
        ...
        self.vfe_layers = nn.ModuleList(vfe_layers)
        self.num_vfe = len(vfe_layers)
        self.vfe_scatter = DynamicScatter(initial_voxel_size, point_cloud_range,
                                          (mode != 'max'))
        self.cluster_scatter = DynamicScatter(
            initial_voxel_size, point_cloud_range, average_points=True)
        self.fusion_layer = None
        if fusion_layer is not None:
            self.fusion_layer = MODELS.build(fusion_layer)
