...
@TRANSFORMS.register_module()
class DonutRangeObjectFilter(BaseTransform):
    '''
    Filter objects by the range and remove objects within the inner box.
    Consider objects only outside the inner box.
    '''
    def __init__(self, point_cloud_range: List[float]) -> None:
        self.pcd_range = np.array(point_cloud_range, dtype=np.float32)
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        gt_bboxes_3d = input_dict['gt_bboxes_3d']
        gt_labels_3d = input_dict['gt_labels_3d']
        mask = self.out_range_bev(gt_bboxes_3d, bev_range)
        gt_bboxes_3d = gt_bboxes_3d[mask]
        gt_labels_3d = gt_labels_3d[mask.numpy().astype(bool)]
        # limit rad to [-pi, pi]
        gt_bboxes_3d.limit_yaw(offset=0.5, period=2 * np.pi)
    ...
    def out_range_bev(
            self, gt_bboxes_3d: Union[LiDARInstance3DBoxes, DepthInstance3DBoxes],
            box_range: Union[Tensor, np.ndarray, Sequence[float]]) -> Tensor:
        _out_range_flags = ((gt_bboxes_3d.bev[:, 0] < box_range[0]) |
                            (gt_bboxes_3d.bev[:, 1] < box_range[1]) |
                            (gt_bboxes_3d.bev[:, 0] > box_range[2]) |
                            (gt_bboxes_3d.bev[:, 1] > box_range[3]))
        return _out_range_flags
...
@TRANSFORMS.register_module()
class DistanceConditionalSampling(BaseTransform):
    """
    Sampling points using distance based probability.
    """
    def __init__(self, factor: float = 5.0, num_pts: int = 1000):
        self.factor = factor
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        points = pts.tensor
        assert self.num_pts <= points.shape[0]
        sampling_prob = (torch.norm(points, dim=1)) ** -self.factor
        sampling_prob /= torch.sum(sampling_prob).float()
        idx = sampling_prob.multinomial(self.num_pts)
        input_dict['points'] = pts.new_point(points[idx, :])
        return input_dict

    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f'(distance conditional sampling factor = {self.factor} with {self.num_pts} points)'
        return repr_str

@TRANSFORMS.register_module()
class DonutCropBox(BaseTransform):
    """
    Crop points within the given box and remove points within the inner box.
    """
    def __init__(self, point_cloud_range: List[float]) -> None:
        self.pcd_range = np.array(point_cloud_range, dtype=np.float32)
    ...
    def transform(self, input_dict: dict) -> dict:
        points = input_dict['points']
        mask = self.out_range_3d(points)
        clean_points = points[mask]
        input_dict['points'] =  clean_points
        return input_dict
    
    def out_range_3d(self,points):
        _out_range_flags = ((points.tensor[:, 0] < self.pcd_range[0]) |
                            (points.tensor[:, 1] < self.pcd_range[1]) |
                            (points.tensor[:, 2] < self.pcd_range[2]) |
                            (points.tensor[:, 0] > self.pcd_range[3]) |
                            (points.tensor[:, 1] > self.pcd_range[4]) |
                            (points.tensor[:, 2] > self.pcd_range[5]))
        return _out_range_flags

    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f'(Donut Cropbox point_cloud_range={self.pcd_range.tolist()})'
        return repr_str
