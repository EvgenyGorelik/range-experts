...
def reduce_LiDAR_beams(pts, reduce_beams_to=32):
    """only works for nuScenes dataset
    from https://github.com/mit-han-lab/bevfusion/blob/326653dc06e0938edf1aae7d01efcd158ba83de5/mmdet3d/datasets/pipelines/loading_utils.py#L57
    """
    # print(pts.size())
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'lidar_sweeps' not in results:
            ...
            for idx in choices:
                ...
                if self.reduce_beams is not None:
...
class LoadPointsFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        lidar2ego = results['lidar_points']['lidar2ego']
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'reduce_beams={self.reduce_beams})'
        return repr_str
    ...
    def __call__(self, results):
        ...
        try:
            if "sweeps" in results:
                print("lidar_sweeps", results['lidar_sweeps'])
                pts_filename = results['lidar_sweeps']['lidar_points']['lidar_path']
                reference_SE3_ego = results['lidar_sweeps']['lidar2ego']
                timestamp_delta = results['timestamp']
                points = self._load_points(pts_filename, reference_SE3_ego, timestamp_delta)
            else:
                pts_filename = results['lidar_path']
                pts = read_feather(pts_filename, columns=["x", "y", "z", "intensity", "laser_number"])
                points = pts.loc[:, ['x', 'y', 'z', 'intensity']].to_numpy().astype(np.float32)
                points[..., 3] = pts.loc[:, "intensity"].to_numpy() * 1.0 / 255
...
class LoadPointsFromMultiSweepsFeather(LoadPointsFromMultiSweeps):
    ...
    def __call__(self, results):
        ...
        for i in range(len(results['lidar_sweeps'])):
            result = results['lidar_sweeps'][i]
            pts_filename = result['lidar_points']['lidar_path']
            reference_SE3_ego = result['lidar_points']['lidar2ego']
            timestamp_delta = result['timestamp']
            points = self._load_points(pts_filename, reference_SE3_ego, timestamp_delta)
