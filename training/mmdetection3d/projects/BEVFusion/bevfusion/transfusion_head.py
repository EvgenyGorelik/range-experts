...
@MODELS.register_module()
class EqualConvFuser(nn.Module):
    def __init__(self, in_channels: int, mid_channels: int, out_channels: int) -> None:
        ...
        self.conv_fuser = nn.Sequential(
            nn.Conv2d(
                len(in_channels) * mid_channels, out_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),
        )
        self.pre_convs = nn.ModuleList()
        for in_channel in in_channels:
            self.pre_convs.append(nn.Sequential(nn.Conv2d(in_channel, mid_channels, 1, bias=False),
                                                nn.BatchNorm2d(mid_channels),
                                                nn.ReLU(True)))

    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        inputs = [pre_conv(input) for pre_conv, input in zip(self.pre_convs, inputs)]
        return self.conv_fuser(torch.cat(inputs, dim=1))
...
class TransFusionHead(nn.Module):
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        loss_heatmap = self.loss_heatmap(
            ...
            heatmap.float().to(preds_dict['dense_heatmap'].device),
        ...
        for idx_layer in range(
            ...
            layer_loss_cls = self.loss_cls(
                ...
                layer_labels.to(layer_cls_score.device),
                layer_label_weights.to(layer_cls_score.device),
            ...
            layer_loss_bbox = self.loss_bbox(
                preds,
                layer_bbox_targets.to(preds.device),
                layer_reg_weights.to(preds.device),
                avg_factor=max(num_pos, 1))
